...
public class ListCertificatesResponderImplTest {
    ...
    public void listCertificatesWithNoCertificates() {
        ...
        ListCertificatesRequestType parameters = createListCertificatesRequest(civicRegistrationNumber, certificateTypes);

        ListCertificatesResponseType response = responder.listCertificates(null, parameters);

        verify(certificateService).listCertificates(civicRegistrationNumber, certificateTypes);

        assertEquals(0, response.getMeta().size());
        assertEquals(OK, response.getResult().getResultCode());
    }

    private ListCertificatesRequestType createListCertificatesRequest(String civicRegistrationNumber, List<String> types) {
