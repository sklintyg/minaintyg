...
@Slf4j
public class MinaIntygLoggingSessionRegistryImpl<T extends Session> extends
    SpringSessionBackedSessionRegistry<T> {
  ...
  private MinaIntygUser getUser(Session session) {
    ...
    final var principal =
        authenticator != null ? authenticator.getAuthentication().getPrincipal() : null;
