			...
			return [
					...
					function($modal) {
						...
						function _showDialog(scope, options) {
							...
							options.dialogId = (options.dialogId == undefined) ? "id" + Math.floor(Math.random() * 11)
									: options.dialogId;
							options.button1text = (options.button1text == undefined) ? "common.ok"
									: options.button1text;
							options.button2text = (options.button2text == undefined) ? "common.cancel"
									: options.button2text;
							options.button3text = (options.button3text == undefined) ? "common.nodontask"
									: options.button3text;
							options.button1id = (options.button1id == undefined) ? "button1"
									+ Math.floor(Math.random() * 11) : options.button1id;
							options.button2id = (options.button2id == undefined) ? "button2"
									+ Math.floor(Math.random() * 11) : options.button2id;
							options.button3id = (options.button3id == undefined) ? "button3"
									+ Math.floor(Math.random() * 11) : options.button3id;
							options.autoClose = (options.autoClose == undefined) ? true : options.autoClose;

							var DialogInstanceCtrl = function($scope, $modalInstance, dialogId, titleId, bodyTextId,
