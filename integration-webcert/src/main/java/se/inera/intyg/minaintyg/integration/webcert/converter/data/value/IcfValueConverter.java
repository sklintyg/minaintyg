...
public class IcfValueConverter extends AbstractValueConverter {
  ...
  private static CertificateQuestionValueText getTextWithIcfCodes(
    ...
    final var collectionsLabel =
        ((CertificateDataConfigIcf) element.getConfig()).getCollectionsLabel();
    final var formattedString = buildFormattedString(icfCodes, text, collectionsLabel);

    return CertificateQuestionValueText.builder().value(formattedString).build();
  }

  private static String buildFormattedString(
      List<String> icfCodes, String text, String collectionsLabel) {
