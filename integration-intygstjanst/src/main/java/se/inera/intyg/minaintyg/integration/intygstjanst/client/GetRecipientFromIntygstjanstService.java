...
import se.inera.intyg.minaintyg.integration.intygstjanst.client.dto.CertificateRecipientResponseDTO;

@Service
public class GetRecipientFromIntygstjanstService {
  ...
  public GetRecipientFromIntygstjanstService(
  ...
  public CertificateRecipientResponseDTO get(GetCertificateRecipientIntegrationRequest request) {
    return webClient.post().uri(uriBuilder -> uriBuilder
            .scheme(scheme)
            .host(baseUrl)
            .port(port)
            .path(endpoint + "/recipient")
            .build())
        .body(Mono.just(request), GetCertificateRecipientIntegrationRequest.class)
        .header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
        .retrieve()
        .bodyToMono(CertificateRecipientResponseDTO.class)
        .share()
        .block();
  }
}
